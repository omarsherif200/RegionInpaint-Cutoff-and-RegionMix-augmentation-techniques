...
def get_vgg16_model(weights):
    """ Creates a vgg model that returns a list of intermediate output values (feature values)
    where these features will be used to calculate perceptual and style loss functions."""
...
class StyleModel(Model):
    """ Build a model that returns the style and content tensor ."""
...
def gram_matrix(x, norm_by_channels=False):
    """Calculate the gram matrix which is used to calculate style loss function"""
    # Assertions on input
...
def loss_perceptual(vgg_out, vgg_gt, vgg_comp):
    """Perceptual loss based on VGG16 features, The first part of the perceptual loss tries to minimize the semantic differences
    between the original ground truth image and the reconstructed one, while the second part focuses only on minimizing the semantic differences
    between the reconstructed missing pixels with their corresponding ones in the original image."""
    l = 0
...
def loss_style(vgg_out, vgg_gt, vgg_comp):
    """ the first term in style_score focuses on minimizing the distance between the style of the original ground truth and the reconstructed one
    while the second term focuses only on minimizing the distance between the style of the reconstructed missing pixels and their corresponding pixels
...
def loss_tv(inv_mask, y_comp):
    """Total variation loss, used for smoothing  to ensure the smoothness in the reconstructed missing pixels obtained by the inpainting network """
...
def loss_total(model, inputs, targets, vgg16, training=True):
    """
    total loss which is a combination between those different loss functions with different weights as given by Liu G et al.
    """
    mask = inputs[1]
